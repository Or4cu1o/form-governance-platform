...
- [ ] T020 Criar migração `apps/api/prisma/migrations/<nova>_create_audit_analytics_schemas/migration.sql`: `CREATE SCHEMA audit`, `CREATE SCHEMA analytics`, role `tableau_ro` ainda sem privilégio
- [ ] T021 Criar migração `apps/api/prisma/migrations/<nova>_add_catalog_and_versions/migration.sql` criando explicitamente `indicator_catalog`, `indicator_response_version` (com o índice único parcial de T016), `report_submission`, `audit.access_log`, `export_seal` e `export_seal_revocation` — a lista é exaustiva de propósito: T035 revoga DML sobre três destas e falha se alguma não existir
- [ ] T022 Criar migração `apps/api/prisma/migrations/<nova>_backfill_legacy_versions/migration.sql`: para cada `IndicatorResponse` existente, exatamente **uma** versão com `originLegacy = true` e `validTo = NULL` — nenhuma versão intermediária é sintetizada, história não se inventa
- [ ] T023 Criar migração `apps/api/prisma/migrations/<nova>_backfill_report_submissions/migration.sql` derivando `ReportSubmission` dos campos únicos existentes de `ReportInstance`, preservando o que houver e marcando a origem como legada
- [ ] T024 Criar migração `apps/api/prisma/migrations/<nova>_backfill_catalog_entries/migration.sql` criando entradas de catálogo a partir dos indicadores existentes, para que `catalogEntryId` possa virar obrigatório sem perda
- [ ] T025 Criar migração `apps/api/prisma/migrations/<nova>_migrate_audit_log_to_audit_schema/migration.sql` movendo `audit_log` e ajustando o trigger de `20260713195352_add_audit_trigger` ao schema novo

### Trilha e contexto de auditoria

- [ ] T026 [TEST] Criar `apps/api/src/common/services/audit-context.service.spec.ts` cobrindo: escrita sem contexto é **rejeitada**, nunca gravada em silêncio
- [ ] T027 Criar `apps/api/src/common/services/audit-context.service.ts` com `runWithAuditContext({ userId, sourceIp, userAgent, origin, requestId }, tx)` sobre `AsyncLocalStorage`
- [ ] T028 Criar `apps/api/src/common/interceptors/audit-context.interceptor.ts` que popula o contexto a partir da requisição, e registrá-lo em `apps/api/src/app.module.ts`
- [ ] T028a [TEST] Criar `apps/api/src/common/services/system-actor.spec.ts`: escrita originada de cron ou de seed grava `actorKind = SISTEMA` com `origin` identificável, e **jamais** autoria nula (FR-071, Princípio I)
- [ ] T028b Criar `apps/api/src/common/services/system-actor.ts` e envolver `apps/api/src/lifecycle/lifecycle-cron.service.ts` e `apps/api/prisma/seed*.ts` em `runWithAuditContext` com ator de sistema — sem isto a abertura automática do período passa a ser rejeitada por T026, derrubando FR-010 e SC-001
- [ ] T029 [TEST] Criar `apps/api/src/audit/access-log.service.spec.ts` verificando gravação de leitura com filtros na íntegra, escopo e volume (FR-073)
- [ ] T030 Criar `apps/api/src/audit/access-log.service.ts` e `apps/api/src/audit/audit.module.ts`
- [ ] T030a Criar `apps/api/src/reports/absence.util.ts` com as cinco representações físicas de `data-model.md` e o `.spec.ts` correspondente, garantindo que nenhuma colapse em outra — é fundação, não código de US1: T110 (auditoria) e T149 (views analíticas) dependem dela

### Sessão: fechar a violação do Princípio V

- [ ] T031 [TEST] Alterar `apps/api/src/auth/auth.controller.spec.ts` para exigir emissão de cookie `HttpOnly; Secure; SameSite=Strict` e recusa de escrita sem token anti-CSRF válido
- [ ] T032 Alterar `apps/api/src/auth/auth.controller.ts` e `apps/api/src/auth/jwt.strategy.ts` para ler o token do cookie, emiti-lo no login e invalidá-lo no logout; adicionar guard anti-CSRF em `apps/api/src/common/guards/`; registrar em `AccessLog` toda autenticação, bem-sucedida e falha (FR-073)
- [ ] T033 Alterar `apps/web/src/api/` e `apps/web/src/context/` para abandonar `sessionStorage`, enviar credenciais por cookie e propagar o token anti-CSRF; atualizar `apps/web/src/pages/LoginPage.test.tsx` e os testes de contexto afetados
- [ ] T034 Atualizar `apps/web/SECURITY-NOTES.md` **no mesmo commit** que conclui T032–T033, removendo o risco aceito de `sessionStorage` — o documento não pode sobreviver à correção que ele descreve

### Revogação de DML (o passo final)

- [ ] T035 Criar migração `apps/api/prisma/migrations/<nova>_revoke_dml_on_append_only/migration.sql`: `REVOKE UPDATE, DELETE` da role da aplicação sobre `indicator_response_version`, `validation_record`, `audit.audit_log`, `audit.access_log`, `export_seal` e `export_seal_revocation`
- [ ] T036 [TEST] Criar `apps/api/src/prisma/append-only.spec.ts` provando que `UPDATE` e `DELETE` diretos nessas **seis** tabelas, **com a credencial da aplicação**, falham por privilégio, e que a tentativa fica registrada (FR-047, FR-070, quickstart V5)
- [ ] T037 [TEST] Criar `apps/api/src/prisma/no-physical-delete.spec.ts` provando que nenhuma rota da aplicação consegue excluir fisicamente usuário, unidade ou evidência (FR-067)
...
- [ ] T166 **CRÍTICO** `apps/api/src/prisma/prisma.service.ts:18` define `runWithAuditActor`, e uma varredura do repositório inteiro não encontra **nenhum chamador**. Toda escrita passa pelo cliente comum, `app.current_user_id` nunca é definida, e o gatilho grava `user_id = NULL` em cada linha de `audit_logs`. Encaminhar toda escrita auditada por ele — ou substituí-lo pelo `runWithAuditContext` de T027, que é o mesmo mecanismo com contexto mais rico — e criar `apps/api/src/prisma/audit-actor.spec.ts` provando que uma escrita sem ator é **rejeitada**, jamais gravada anônima (Princípio I, FR-071, `contradicts`)
- [ ] T167 **CRÍTICO** `prisma/migrations/20260713195352_add_audit_trigger/migration.sql` instala `fn_write_audit_log` em apenas duas tabelas — `indicator_responses` e `evidence_files`. `users`, `units`, `user_unit_accesses`, `form_templates`, `form_topics`, `form_indicators`, `system_settings` e `validation_records` não têm gatilho algum: a alteração administrativa do cenário US5-5 não deixa rastro nenhum hoje. Criar migração estendendo o gatilho às tabelas faltantes, com teste de cobertura que falhe se uma tabela auditável ficar de fora (FR-069, cenário US5-5, `missing`). Pré-requisito real de T094 e T097, que assumem uma trilha que não existe
...
- [ ] T171 **BAIXO** `apps/api/src/main.ts:41-44` cai em `enableCors()` sem opções — `Access-Control-Allow-Origin: *` — quando `CORS_ORIGIN` está vazia ou ausente. O comentário no código declara o compromisso como temporário até o frontend ter domínio fixo. A Fase 2 dá domínio fixo ao frontend e move a sessão para cookie; cookie com CORS aberto é combinação que não se sustenta. Tornar `CORS_ORIGIN` obrigatória em `env.validation.ts` junto com T005 e remover o ramo aberto (`contradicts`, acopla a T032)
