...
**Estado atual**: **implementado nesta sessão** (2026-08-10).

### Testes

- [x] T144 [P] [US8] [TEST] Criar `apps/api/src/analytics/analytics-views.spec.ts`: relatório não concluído não aparece em **nenhuma** view (cenário US8-1, FR-115)
- [x] T145 [P] [US8] [TEST] Estender `apps/api/src/analytics/analytics-views.spec.ts`: média sobre recorte com células ausentes ignora as ausentes no denominador, e o `n` bate com o da Área de Auditoria (cenário US8-5, SC-019)
- [x] T146 [P] [US8] [TEST] Estender `apps/api/src/analytics/analytics-views.spec.ts`: meta alterada após a emissão não altera o que o BI lê daquele relatório (cenário US8-3, FR-117)
- [x] T147 [P] [US8] [TEST] Criar `apps/api/src/analytics/analytics-privileges.spec.ts`: `INSERT`/`UPDATE`/`DELETE` como `tableau_ro` falham por privilégio, e a role da aplicação **não** tem `SELECT` em `analytics` (cenário US8-4, FR-118)
- [x] T148 [P] [US8] [TEST] Criar `apps/api/src/analytics/evidence-resolver.spec.ts`: segunda utilização do mesmo token apresenta tela de expiração, não erro cru; nenhuma resposta contém o endereço do armazenamento (cenários US8-6, US8-8)
- [x] T148a [P] [US8] [TEST] Criar `apps/api/src/analytics/drill-down.spec.ts`: a partir de `v_report_fact.calculated_value` a cadeia é percorrível até a evidência — decomposição do cálculo (variáveis e expressão congeladas) → histórico de autoria e alteração → `v_evidence_link` → resolver → arquivo, cada elo alcançável em no máximo 3 passos (FR-119, SC-006)

  **Nota de execução (T144-T148a)**: `analytics-views.spec.ts`, `analytics-privileges.spec.ts` e `drill-down.spec.ts` são testes de integração contra Postgres real (mesmo padrão de `prisma/append-only.spec.ts` — role da aplicação via `APP_DATABASE_URL`, aqui somados `TABLEAU_RO_DATABASE_URL`/`DATABASE_URL`), não executáveis neste sandbox (rede Docker isolada, mesma limitação documentada desde T020-T025). `evidence-resolver.spec.ts` e `evidence-token.util.spec.ts` (novo, cobertura direta da assinatura HMAC) rodam com mocks, sem banco — 12 testes, todos verdes localmente.

### Implementação

- [x] T149 [US8] Criar migração `apps/api/prisma/migrations/20260810095000_create_analytics_views/migration.sql` com `v_report_fact`, `v_absence_semantics`, `v_indicator_dim`, `v_unit_dim` e `v_evidence_link` conforme [contracts/analytics-layer.md](./contracts/analytics-layer.md)
- [x] T150 [US8] Incluído na mesma migração `v_absence_semantics.counts_in_denominator` — `false` para toda forma de ausência, `true` apenas para `VALOR` e `ZERO_MEDIDO` (FR-116)
- [x] T151 [US8] Criar migração `apps/api/prisma/migrations/20260810100000_grant_tableau_ro/migration.sql`: `GRANT SELECT` a `tableau_ro` **apenas** em `analytics` (+ `ALTER DEFAULT PRIVILEGES` para view futura), e `REVOKE` explícito de `formops_app` sobre `analytics` (defesa em profundidade — nunca houve `GRANT` prévio, `20260809090000_revoke_dml_on_append_only` só tocou `public`/`audit`)
- [x] T152 [US8] Criar `apps/api/src/analytics/evidence-resolver.service.ts` e `apps/api/src/analytics/analytics.module.ts` com token HMAC-SHA256 de uso único e vida curta
- [x] T153 [US8] Criar `GET /analytics/evidence/:token` em `apps/api/src/analytics/analytics.controller.ts`, respondendo de forma indistinguível para token consumido, expirado e inválido
- [x] T154 [US8] Registrar `AccessLog` em **todo** acesso originado do BI — bem-sucedido, expirado ou já consumido — em `apps/api/src/analytics/evidence-resolver.service.ts` (cenário US8-7)
- [x] T155 [P] [US8] Criar `apps/web/src/pages/EvidenceExpiredPage.tsx` e `EvidenceExpiredPage.test.tsx`: tela amigável de expiração, nunca erro cru
- [x] T156 [US8] Implementar o marcador de última carga e a capacidade de recarga completa da camada analítica (FR-121): view `analytics.v_load_marker` (migração `20260810101000_add_analytics_load_marker`) exposta ao BI com o instante da carga corrente, e `apps/api/src/analytics/analytics-reload.service.ts` (com `.spec.ts`, mockado, verde). Enquanto as views forem não materializadas a "carga" é a própria leitura e o marcador reflete o instante da consulta — mas o contrato exposto ao painel é o mesmo que servirá depois da materialização, para que materializar não quebre painel
- [x] T156a [P] [US8] Atualizado `specs/001-plataforma-formops-base/contracts/analytics-layer.md` com o marcador de T156, confirmando que as views permanecem não materializadas até haver medição real (research.md D11)

  **Desvios e decisões registrados nesta sessão**:
  - **Rota sem prefixo `api/`**: os contratos descrevem `GET /api/analytics/evidence/:token`, mas nenhum controller existente (inclusive `VerificationController`, `@Controller('public')`) usa prefixo `api/` — não há `setGlobalPrefix` em `main.ts`, e o web client chama rotas como `/audit/query` diretamente. `AnalyticsController` foi ajustado para `@Controller('analytics')` (rota real `/analytics/evidence/:token`), mantendo paridade com o padrão já em produção em vez do texto literal do contrato.
  - **`unit_level_at_period`/`v_unit_dim.unit_level` usam o nível corrente de `Unit`, não histórico**: não existe tabela de versionamento de `Unit.level` no modelo — a mesma simplificação já em vigor em `audit-query.service.ts` (que também lê `unit.level` corrente). Documentado inline na migração `20260810095000_create_analytics_views` como `[verificar na implementação]`: manter os dois lados (Auditoria e BI) igualmente "correntes" é o que preserva a paridade exigida por FR-122 hoje: introduzir histórico só de um lado quebraria a paridade em vez de resolvê-la.
  - **Token do resolver de evidência não é calculado dentro da VIEW**: o segredo HMAC (`EVIDENCE_RESOLVER_HMAC_SECRET`, já reservado em `env.validation.ts` desde a Fase 1) nunca entra no banco — a assinatura roda inteiramente em Node (`evidence-token.util.ts`). Isso exige um estado real que uma `VIEW` pura não pode manter (uso único, expiração) — nova tabela `evidence_access_tokens` (migração `20260810094000_add_evidence_access_tokens`, model `EvidenceAccessToken` em `schema.prisma`) guarda o token já assinado (mesmo precedente de `export_seal.verification_code`: já não-enumerável). `v_evidence_link` faz `LEFT JOIN LATERAL` no token vigente (não consumido, não expirado); fica `NULL` até a próxima recarga emitir um.
  - **"Rotina de recarga" (T156) hoje reemite tokens de evidência**: como a camada permanece não materializada (D11), não há nada além da leitura em si para "recarregar" nas views de fato — o único estado real fora da leitura direta é o token de evidência. `AnalyticsReloadService.reload()` (também acionado por `@Cron('0 * * * *')`) cobre exatamente esse caso; documentado explicitamente no contrato (T156a) para que uma futura materialização real não precise redesenhar o papel desta rotina, só trocar a fonte de `loaded_at`.
  - **Provisionamento de `tableau_ro` nunca tinha sido escrito**: `T020` criou a role `NOLOGIN`, mas nenhum script equivalente a `provision-app-role.ts` existia. Criado `apps/api/scripts/provision-tableau-ro.ts` (mesmo padrão, `TABLEAU_RO_DB_PASSWORD` nova, fora de SQL versionado) e ligado em `scripts/manage.js` (`provisionTableauRo()`, chamado nos três pontos onde `provisionAppRole()` já rodava) e em `apps/api/docker-entrypoint.sh`. `TABLEAU_RO_DATABASE_URL` derivada automaticamente em `scripts/manage.js`, mesma lógica de `APP_DATABASE_URL` — usada pelos testes de integração T144-T148a.
...
- [x] T157 [P] [TEST] `apps/api/src/reports/invariants.spec.ts` consolida as cinco invariantes num único portão nomeado — cada teste chama diretamente a implementação real (`absence.util.ts`, `AuditQueryService`, `computeArtifactDigest`, `FormIndicatorsService.assertBalanced`), não recria a lógica. As cinco já tinham cobertura dispersa nos seus próprios módulos; este arquivo é o rastro único que uma auditoria de conformidade pode citar. 6/6 testes verdes.
- [x] T157a [P] [TEST] `apps/web/src/test/state-labels.test.tsx` (12 testes) varre `AbsenceLegend`, `SparseMatrix`, `StatusBadge`/`AdminCatalogPage`, `AuditPage`, `VerifyPage` (5 vereditos) e `EvidenceExpiredPage`. Achado real: o marcador `▲` de outlier em `SparseMatrix.tsx` só tinha `title=`, não confiável em leitor de tela nem em touch — corrigido com `aria-label` explícito (regressão coberta no próprio teste). Nenhum outro caso de "só cor" encontrado.
- [x] T158 [P] Guarda pericial já existia (T050/`antivirus.service.ts`, `forensicHoldUntil` carimbado automaticamente); a lacuna real era a liberação antecipada. Implementado `POST evidence-files/:id/forensic-release` (`@Roles(ADMINISTRADOR)`, não `/api/evidence/:id/...` — mesma correção de rota das Fases 9/10, sem prefixo `api/` neste codebase) em `evidence.controller.ts`/`evidence.service.ts`, com `ForensicReleaseDto` (motivo obrigatório) e três novos campos em `EvidenceFile` (`forensicHoldReleasedByUserId/At/Reason`, migração `20260810110000_add_forensic_hold_release`) — a própria linha, capturada pelo gatilho de auditoria já existente (T166/T167) em `evidence_files`, é o registro de autor/motivo/data. 11 testes novos entre `evidence.service.spec.ts`/`evidence.controller.spec.ts`.
- [x] T159 [P] Bloqueio por conta implementado do zero em `auth.service.ts` (`User.failedLoginAttempts`/`accountLockedUntil`, migração `20260810120000_add_account_and_ip_lockout`) — mecanismo primário, threshold 5, janela 15 min. Camada secundária por IP nova (`IpLoginLockout`, tabela dedicada não coberta pelo gatilho de auditoria — mesmo precedente de `evidence_access_tokens`), threshold 30, janela 5 min, com lista de exceção `Unit.knownEgressIps` (exposta via `CreateUnitDto`/`UpdateUnitDto` para o admin poder configurar — sem essa exposição o campo do schema ficaria inoperável). 9 testes novos em `account-lockout.spec.ts`, incluindo o cenário literal do quickstart V16 (segundo usuário do mesmo endereço continua autenticando).
- [x] T159a [P] Já implementado (achado, não lacuna): `sanitizeHeaderValue()` em `email-templates.util.ts` já remove `\r\n` de `unit.sigla` antes de compor o assunto, em todos os 5 templates, com teste dedicado (`email-templates.util.spec.ts:62`, "strips CRLF... header injection defense"). Nenhum arquivo `subject.util.ts` separado criado — a sanitização já vive onde o assunto é montado, e duplicar a função só para bater o nome do arquivo do texto original da tarefa violaria DRY sem ganho.
- [x] T160 `docker-compose.yml`: WAL archiving contínuo no serviço `postgres` (`archive_mode=on`, `archive_timeout=300`, volume `formops_wal_archive`). `scripts/backup-restore.sh` (`npm run backup:base`/`backup:drill`) automatiza base backup e um exercício de restauração completo num container descartável, sempre anexando o resultado a `docs/backup-restore-drill-log.md` — nunca sobrescrito. Política de retenção da trilha documentada em `docs/backup-restore.md` (piso = `SystemSetting.evidenceRetentionYears`, expurgo só manual fora da aplicação, confirmado pelo `REVOKE` de T035). **Não executado neste sandbox** (mesma limitação de rede Docker isolada de T020 em diante) — primeira entrada do log registra esse estado e a ação pendente.
- [x] T161 [P] `apps/api/scripts/measure-canonical-query-performance.ts` (`npm run measure:canonical-query`) mede a primeira página da consulta canônica via `AuditQueryService` real (bootstrap `NestFactory.createApplicationContext`), limiar 3s, com `SCALE_LABEL` para rotular execuções em escalas diferentes. **Não executado neste sandbox** — sem Postgres real alcançável e `seed-demo.ts` hoje cobre 6 meses, abaixo dos 24 exigidos. Registrado em `research.md` D11: nenhum número real ainda existe, a decisão de materializar segue **não antecipada**.
- [x] T161a [P] `apps/api/scripts/measure-monthly-close-load.ts` (`npm run measure:monthly-close-load`) simula concorrência real via `IndicatorResponsesService.updateValues`/`ValidationService.validateIndicator` contra o acervo semeado, com `CONCURRENCY`/`UNIT_TARGET` parametrizáveis. **Não executado neste sandbox**, mesma limitação. Registrado em `research.md` D11 junto com T161.
- [x] T162 [P] `CLAUDE.md`: nova seção "Módulos do backend" listando `catalog`/`audit`/`sealing`/`verification`/`analytics`; linha "Autenticação" da tabela Stack corrigida de "JWT (Bearer)" (desatualizado desde a migração para cookie `httpOnly`+CSRF) para o mecanismo real, incluindo o bloqueio automático desta fase. Achado adjacente corrigido: `CLAUDE.md` referenciava `apps/web/SECURITY-NOTES.md`, que não existe no repositório — removida a referência fantasma.
- [x] T163 [P] `apps/api/SECURITY-NOTES.md`: reavaliação honesta, não um "risco resolvido" fabricado — nodemailer sem mudança de superfície (quarentena/assinatura protegem o pipeline de evidências, não o de e-mail); multer com risco residual **reduzido mas não eliminado** (as duas camadas novas atuam depois do parsing `multipart/form-data` onde os CVEs de DoS vivem; plano de correção continua sendo o upgrade de framework 10→11).
- [x] T164 Gate final: **backend** — build limpo; 529/529 testes unitários verdes (8 suítes falham só no carregamento, mesma limitação de Postgres documentada desde T020, contagem inalterada); lint 0 erros (14 warnings pré-existentes em scripts de seed); cobertura Statements 83,73% / Functions 86,85% / Lines 84,67% / **Branches 78,84%** (marginalmente abaixo de 80%, sem `coverageThreshold` configurado — nenhum gate automático bloqueado; registrado aqui por transparência, não perseguido com teste artificial). **Frontend** — build limpo; 262/262 testes verdes (63 arquivos); lint 0 erros (4 warnings pré-existentes); cobertura Statements/Lines 93,53%, Branches 82,59%, **Functions 63,45%** (abaixo de 80% — característica estrutural de componentes React ricos em pequenas funções/handlers, sem `coverageThreshold` configurado, não uma regressão desta fase).
- [x] T165 `specs/001-plataforma-formops-base/quickstart-results.md`: os 17 cenários percorridos por revisão de código + suíte automatizada (não execução ao vivo — mesma limitação de sandbox). 5/17 (V2, V6, V9, V14, V16) têm a lógica central inteiramente provada sem depender de infraestrutura externa; os outros 12 têm cobertura de teste para a lógica que a aplicação controla, mas exigem Postgres/MinIO/ClamAV/Docker reais para a garantia de infraestrutura (Object Lock, ClamAV, rede) que só um ambiente vivo confirma. Nenhum cenário marcado sem uma referência concreta de código/teste citada.
